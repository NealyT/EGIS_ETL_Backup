...
def load_urls_from_db(host, dbname, user, password):
    ...
    try:
        ...
        where_clause = "where acronym = 'NCF'"
        # where_clause = ""
        ...
        for child in urlJson["layers"]:
            ...
            try:
                ...
                if hasattr(child_layer, "GeoJSON"):
                    ...
                    saveGeoJson(standard_name, child_layer, output_location)
                else:
                    ...
                    if jsonChild.get("features") is not None:
                        child_data = {
                            "type": "FeatureCollection",
                            "features": jsonChild["features"]  # Assuming 'featureset' is a list of GeoJSON features
                        }
                    else:
                        print(json.dumps(esrijson_data, indent=4))
                        print("object has no features")

                    saveJson(standard_name, child_layer, output_location)
                    convertEsriJsonToGeoJson(ogr_location, output_location, standard_name)
...
def convertEsriJsonToGeoJson(ogr_location, output_location, name):
    ...
    print(f"Converting JSON TO GEOJSON: {output_location}")
