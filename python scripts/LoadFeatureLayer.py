...
def load_data(base_url, product, output_location, urlJson, host,port, dbname, user, password, table_data={}, post_data=False ):
      ...
      try:
        ...
        if post_data:
          if file_name is not None:
            ...
            postGIS(file_name, table_name,host,port,dbname, user_name, password)
      except Exception as e:
        print(e)
...
def convertEsriJsonToGeoJson(output_location, name):
  file_path = os.path.join(output_location,  f"{name}.geojson")
  file_path2 = os.path.join(output_location, f"{name}.json")
  ogr2ogrpath = r"C:\Program Files\QGIS 3.36.3\bin\ogr2ogr.exe"
  command = [
    ogr2ogrpath,
...
def postGIS(file_path, table_name, host,port, dbname, user, password ):
  check_schema(host,dbname,user, password)
...
if __name__ == "__main__":
  ...
  file_path = os.path.join(output_location, "table_info.csv")
  if os.path.exists(file_path):
  ...
  for index, row in df.iterrows():
    ...
    try:
      ...
      output_location = os.path.join(output_location, acronym)
      if not os.path.exists(output_location):
        os.makedirs(output_location)
      table_data = load_data(url, acronym, output_location, urlJson, host, dbname, user_name, password,
                             {}, post_data=True)
      dumpTableData(acronym,r"C:\data\bah", "table_info", table_data)
    except Exception as e:
        print(e)
